...
def plot_surprise_comparison(unified_network: Network, separate_network: Network,
                                save_path: str = None):
    ...
    axes[0].plot(trials, unified_surprise_a + unified_surprise_b, 'purple',
                 linewidth=1.5, alpha=0.7, label='Unified')
    axes[0].plot(trials, separate_surprise_a + separate_surprise_b, 'orange',
...
def plot_network_structure(unified_network: Network, separate_network: Network,
                           save_path: str = None):
    ...
    for title, net, ax in models:
        ...
        try:
            ...
            for parent_idx, edge_data in enumerate(edges_info):
                ...
                if edge_data.value_children is not None:
                    ...
                    for child_idx in children:
                        # Solid black line
                        G.add_edge(parent_idx, child_idx, type='value', color='black', style='solid')
                ...
                if edge_data.volatility_children is not None:
                    ...
                    for child_idx in children:
                        # Dashed black line
                        G.add_edge(parent_idx, child_idx, type='volatility', color='black', style='dashed')

            # --- Layout ---
            pos = {}
            # Level 0: Inputs (0, 1) - Bottom
            pos[0] = (0.3, 0); pos[1] = (0.7, 0)
            
            # Level 1: Hidden States (2, 3)
            pos[2] = (0.3, 1); pos[3] = (0.7, 1)
            
            # Level 2: Local Volatility (4, 5)
            pos[4] = (0.3, 2); pos[5] = (0.7, 2)
            ...
            ax.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, -0.05), fontsize='small', ncol=2, frameon=False)
...
def run_and_plot_comparison(n_trials: int = 175, seed: int = 42, save_dir: str = 'outputs'):
    ...
    plot_surprise_comparison(unified_network, separate_network,
                                save_path=f'{save_dir}/surprise_comparison.png')

    print("Generating network structure plot...")
    plot_network_structure(unified_network, separate_network,
                           save_path=f'{save_dir}/network_structure.png')
