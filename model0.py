...
def create_unified_network(p: dict = PARAMS) -> Network:
   ...
   network = (
        ...
        .add_nodes(n_nodes=2, precision=p["input_precision"])
         ...
         # parent of input 0
        .add_nodes(value_children=0, mean=p["x_a_init"],
                   precision=p["state_precision"], tonic_volatility=p["omega_a"])

        #parent of input 1
        .add_nodes(value_children=1, mean=p["x_b_init"],
                   precision=p["state_precision"], tonic_volatility=p["omega_b"])

        # volatility parent of x_a
        .add_nodes(volatility_children=2, mean=p["x_a_check_init"],
                   precision=p["vol_precision"], tonic_volatility=p["omega_a_check"],
                   volatility_coupling_children=(p["kappa_a"],))
        # parent of x_b
        .add_nodes(volatility_children=3, mean=p["x_b_check_init"],
                   precision=p["vol_precision"], tonic_volatility=p["omega_b_check"],
                   volatility_coupling_children=(p["kappa_b"],))
         # global volatility
        .add_nodes(value_children=[4, 5], mean=p["x_c_init"],
                   precision=p["vol_precision"], tonic_volatility=p["omega_c"],
...
def create_separate_network(p: dict = PARAMS) -> Network:
    ...
    network = (
        ...
        .add_nodes(value_children=0, mean=p["x_a_init"], precision=p["state_precision"], tonic_volatility=p["omega_a"])
        .add_nodes(value_children=1, mean=p["x_b_init"], precision=p["state_precision"], tonic_volatility=p["omega_b"])

        # local volatility coupling on both "branches"
        .add_nodes(volatility_children=2, mean=p["x_a_check_init"], precision=p["vol_precision"], tonic_volatility=p["omega_a_check"],
                   volatility_coupling_children=(p["kappa_a"],))
        .add_nodes(volatility_children=3, mean=p["x_b_check_init"],
                   precision=p["vol_precision"], tonic_volatility=p["omega_b_check"],
                   volatility_coupling_children=(p["kappa_b"],))
        ...
        #  global volatilities
        .add_nodes(value_children=4, mean=p["x_c_init"],
                   precision=p["vol_precision"], tonic_volatility=p["omega_c"],
                   value_coupling_children=(p["alpha_c_a"],))
        .add_nodes(value_children=5, mean=p["x_c_init"],
                   precision=p["vol_precision"], tonic_volatility=p["omega_c"],
...
def generate_observations(network: Network, n_trials: int = N_TRIALS,
                          rng_key=None) -> tuple[Network, jnp.ndarray]:
...
def main():
    ...
    for gen_label in ['unified', 'separate']:
        ...
        for inf_label in ['unified', 'separate']:
            ...
            print(f"  {gen_label:>8s} obs -> {inf_label:>8s} model: "
    ...
    for gen_label in ['unified', 'separate']:
        ...
        print(f"  {gen_label.upper()} obs -> preferred: {winner} "
              f"(delta = {abs(u - s):.1f})")
    ...
    for label, factory in model_factories.items():
        ...
        try:
            factory().input_data(
                input_data=jnp.zeros((10, 2))
            ).plot_network(backend='networkx')
